...
# =============================================================================
# U1 subgroup metrics — users with <= 1 interaction in the training set
# =============================================================================
# Helper functions (not picked up by cluster_info because they are not classes)


def _u1_filter(dataobject, topk):
    """Return (pos_index_u1, pos_len_u1) keeping only rows for u1 users.

    rec.topk row i corresponds to data.user_ids[i] (alignment ensured by
    eval_batch_collect which appends both tensors in the same batch order).

    u1 users: train interaction count <= 1, internal user ID != 0
    (RecBole uses 0 as the padding / unknown token).
    """
    rec_mat   = dataobject.get("rec.topk").numpy()          # (N, max_k+1)
    pos_index = rec_mat[:, :-1].astype(bool)                # (N, max_k)
    pos_len   = rec_mat[:, -1]                              # (N,)
    user_ids  = dataobject.get("data.user_ids").numpy()     # (N,)
    count_map = dict(dataobject.get("data.count_users"))
    train_cnt = np.array([count_map.get(int(uid), 0) for uid in user_ids])
    u1_mask   = (train_cnt <= 1) & (user_ids != 0)
    return pos_index[u1_mask], pos_len[u1_mask]


def _u1_topk_result(metric_name, value, topk, decimal_place):
    """Like TopkMetric.topk_result but returns 0.0 when no u1 rows exist."""
    if len(value) == 0:
        return {f"{metric_name}@{k}": 0.0 for k in topk}
    avg = value.mean(axis=0)
    return {
        f"{metric_name}@{k}": round(float(avg[k - 1]), decimal_place)
        for k in topk
    }


class U1Precision(TopkMetric):
    r"""Precision restricted to users with <= 1 train interaction (u1 group)."""

    metric_type = EvaluatorType.RANKING
    metric_need = ["rec.topk", "data.user_ids", "data.count_users"]

    def calculate_metric(self, dataobject):
        pos_index, _ = _u1_filter(dataobject, self.topk)
        result = Precision.metric_info(self, pos_index)
        return _u1_topk_result("precision_u1", result, self.topk, self.decimal_place)

    def metric_info(self, pos_index):
        return Precision.metric_info(self, pos_index)


class U1Recall(TopkMetric):
    r"""Recall restricted to users with <= 1 train interaction (u1 group)."""

    metric_type = EvaluatorType.RANKING
    metric_need = ["rec.topk", "data.user_ids", "data.count_users"]

    def calculate_metric(self, dataobject):
        pos_index, pos_len = _u1_filter(dataobject, self.topk)
        result = Recall.metric_info(self, pos_index, pos_len)
        return _u1_topk_result("recall_u1", result, self.topk, self.decimal_place)

    def metric_info(self, pos_index, pos_len):
        return Recall.metric_info(self, pos_index, pos_len)


class U1MAP(TopkMetric):
    r"""MAP restricted to users with <= 1 train interaction (u1 group)."""
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = _u1_filter(dataobject, self.topk)
        result = MAP.metric_info(self, pos_index, pos_len)
        return _u1_topk_result("map_u1", result, self.topk, self.decimal_place)

    def metric_info(self, pos_index, pos_len):
        return MAP.metric_info(self, pos_index, pos_len)


class U1NDCG(TopkMetric):
    r"""NDCG restricted to users with <= 1 train interaction (u1 group)."""
    ...
    def calculate_metric(self, dataobject):
        ...
        return _u1_topk_result("ndcg_u1", result, self.topk, self.decimal_place)

    def metric_info(self, pos_index, pos_len):
        return NDCG.metric_info(self, pos_index, pos_len)


class U1MRR(TopkMetric):
    r"""MRR restricted to users with <= 1 train interaction (u1 group)."""
    ...
    def calculate_metric(self, dataobject):
        pos_index, _ = _u1_filter(dataobject, self.topk)
        result = MRR.metric_info(self, pos_index)
        return _u1_topk_result("mrr_u1", result, self.topk, self.decimal_place)
